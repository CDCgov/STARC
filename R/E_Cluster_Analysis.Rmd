...
## Read STARC Data and Separate Based on "Hot spot" or not
...
### Buffer and union "hot spot" hexagons
```{r}
...
### Buffer and union non hot spots hexagons
```{r}
...
### Combine to create cluster outline
```{r}
...
# Identify Hexagons in Clusters


## Non-hotspot Hexagons
```{r}
...
# Classify clusters by R0 and dog pop
```{r}
...
# Create Transmission Zones
grouping clusters together in Transmission Zones, but buffers are not added to statistics or geometries

## Buffer clusters and get transmission zones
```{r}
...
# Cluster summary statisics

## Cluster Level (using hexes in cluster)
```{r}
...
# Regionalize and Prioritize

### Intersect and join with regions
```{r}
...
### Use quantile percentile of Hotspot dog populations to "prioritize"
  * "primary" greater than or equal to  top 75% (of total hot spot dog population)
  * "secondary" greater than or equal to top
  * "tertiary" less than top 25% (AKA bottom 25 %)
```{r}
...
## Summarize zones with cluster stats
```{r}
...
zone_stats4 = zone_stats4 %>% 
  dplyr::select(Zone, 
                ...
                Human_pop, 
                Dog_pop_ALL, 
                D_Pop_FREE=Dog_pop_FREE, 
                area, 
                hu_dens, 
                D_Den_ALL=dog_dens_ALL, 
                D_Den_FREE=dog_dens_FREE,
                C_Pop=Cluster_population,
                C_Dg_POP=Cluster_Dog_pop, 
                C_Area=Cluster_area,
                C_Hu_Dens=Cluster_hu_dens,
                C_Dg_Dens=Cluster_dog_dens,
                C_Rep_O=Cluster_Rep_O, 
                clusters,
                num_Cs=num_clusters,
                Hot_Dg_POP=Hot_Dog_pop,
                hot_spots,
                num_Ht_Sts=num_hot_spots,
                Regions, 
                Reg_Major=Region_majority, 
                Rep_pri, 
                Zone_Label)
...
## Save Zone, Cluster, and Hot Spot Outline Data
  **First merge cluster info with the buffered outlines**
```{r}
getwd()
...
## Save Hexagons Data
  **First include info on cluster ID to individual hexagons**
```{r}
